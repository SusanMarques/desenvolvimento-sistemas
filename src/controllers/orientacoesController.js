...
async function deletarOrientacao(req, res) {
    ...
    try {
        ...
        if(resultado.rowCount === 0) {
            return res.status(404).json({ mensagem: "Orientação não encontrada."})
        }

        return res.status(200).json({ mensagem: "orientação deletado con sucesso." })


    } catch (error) {
        return res.status(500).json({ error: 'Erro ao deletar Orientação.' });
    }
}
